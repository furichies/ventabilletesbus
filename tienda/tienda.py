...
def comprar_billete():
    """
    Realiza el proceso de compra de un billete.
    """
    ...
    response_registro = requests.post(REGISTRO_API_URL, json={'nombre': nombre, 'token': token}, timeout=5)
    ...
    response_plazas = requests.get(PLAZAS_API_URL, timeout=5)
    ...
    response_saldo = requests.get(f'http://cajero:6901/saldo?nombre={nombre}', timeout=5)
    ...
    # Realizar el pago
    response_pago = requests.post(PAGAR_API_URL, json={'nombre': nombre, 'costo': valor_billete}, timeout=5)
